...
const ExperienceManager: React.FC<{ type: 'pro' | 'formation' }> = ({ type }) => {
  ...
  const loadItems = async () => {
    const endpoint = type === 'pro' ? '/professional-experiences' : '/formations';
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // On s'assure que les listes sont des tableaux si on vient d'un textarea
    ...
    <div className="entity-manager">
      <div className="admin-main-header">
        ...
        <button className="primary-button" onClick={() => setEditingItem({
          title: '', period: '', description: '', longDescription: '',
          company: '', institution: '', missions: [],
          hardSkills: [], softSkills: [], type: ''
        })}>
          Ajouter
        </button>
      </div>
            ...
            <tr key={item.id}>
              ...
              <td>
                <button className="action-btn edit-btn" onClick={() => setEditingItem({
                  ...item,
                  // On convertit les tableaux en chaînes pour le premier rendu des textareas si besoin
